...
class MazePlan1 implements Plan
{
  ...
  {  
    ...
    for(int r = 0; r < numSizes; r++)
    {
      ...
      xtstart = int(xwstart / TILE_SIZE);
      ytstart = int(ywstart / TILE_SIZE);
      ...
      for(int i = 0; i < numStartWinRounds; i++)
      {
        ...
        // start Tile Creator starts D_RIGHT or D_DOWN.
        char startCreatorDirection = char(1 + int(random(0,2)));
        
        // winning Tile Creator starts D_LEFT or D_UP.
        char winCreatorDirection = char(3 * int(random(0,2)));
        
        // Add starting and winning Creators.
        creators.add(new Creator(xtstart, ytstart, 0, 0, 2, 4, 2, 2, corridorWidth, avoidProbability, startCreatorDirection, 3 * tilesX / 2));
        creators.add(new Creator(xtwin, ytwin, 0, 0, -2, 4, 2, 2, corridorWidth, avoidProbability, winCreatorDirection, 3 * tilesX / 2));
      ...
      for(int i = 0; i < numRounds; i++)
      {
        ...
        float avoidProbability = 0.4 + 0.55 * float(i) / float(numRounds);
        ...
        for(int j = 0; j < creatorsPerRound; j++)
        {
          ...
          float winBias = random(-0.025, 0.025);
